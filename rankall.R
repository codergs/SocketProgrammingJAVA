    ...
    if(non && out && numcheck){
      ...
      if(length(outcome1[,1])>=num){
        ...
        if(outcome=="heart attack"){ 
          ...
          state.data<-outcome1[order(outcome1[,7],outcome1[,11],decreasing=num2),]
          state.list<-split(state.data,state.data[,7])
          temp<-vector()
          for(i in 1:length(state.list)){
            temp<-append(temp,state.list[[c(i,2)]][num])
          }
        ...
        if(outcome=="heart failure"){ 
          z<-is.na(outcome1[,17])
          outcome1<-outcome1[!z,]   # For cases with non NA values
          state.data<-outcome1[order(outcome1[,17],decreasing=num2),]
          state.list<-split(state.data,state.data[,7])
          temp<-vector()
          for(i in 1:length(state.list)){
            temp<-append(temp,state.list[[c(i,2)]][num])
        ...
        if(outcome=="pneumonia"){ 
          z<-is.na(outcome1[,23])
          outcome1<-outcome1[!z,]   # For cases with non NA values
          state.data<-outcome1[order(outcome1[,23],decreasing=num2),]
          state.list<-split(state.data,state.data[,7])
          temp<-vector()
          for(i in 1:length(state.list)){
            temp<-append(temp,state.list[[c(i,2)]][num])
